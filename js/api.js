$(document).ready(function(){
	...
	$('div.endpoints').on('click','a.call-api',function(){
		...
		if(method=='GET'){
			$('.api-curl').html('curl -s "<a target="_blank" href="http://'+document.location.host+'/'+version+'/'+endpoint.replace('-','/')+'/?'+params+'">http://'+document.location.host+'/'+version+'/'+endpoint.replace('-','/')+'/?'+params+'</a>"');
		} else {
			$('.api-curl').html('curl --data "'+params+'" http://'+document.location.host+'/'+version+'/'+endpoint.replace('-','/')+'/');
		}
		$('div.curl').show();
		...
		$.ajax({
			type: method
			,url: 'http://'+document.location.host+'/'+version+'/'+endpoint.replace('-','/')+'/'
			,data: params
			,dataType: "json"
				...
				if(paging.limit != -1 && paging.limit < paging.total){
					...
					for(var p=0;p<pageLimit;p++){
						if(paging.offset==(p*paging.limit))
							page_html += '<li class="current"><a href="#" class="has-tip call-api" data-method="'+method+'" data-tooltip data-version="'+version+'" data-endpoint="'+endpoint+'" data-offset="'+p*paging.limit+'" data-limit="'+paging.limit+'" title="Page '+p+'">'+p+'</a></li>';
						else
							page_html += '<li><a href="#" class="has-tip call-api" data-tooltip  data-method="'+method+'" data-version="'+version+'" data-endpoint="'+endpoint+'" data-offset="'+p*paging.limit+'" data-limit="'+paging.limit+'" title="Page '+p+'">'+p+'</a></li>';
					}
					if(totalPages>5){
						page_html += '<li>...</li>';
